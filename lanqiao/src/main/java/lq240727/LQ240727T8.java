...
分析题意，显然给定我们的是一棵树。
问题转化为，对于这棵树所有合法的 DFS 序遍历，其中第 i 个位置可能出现多少种不同的值。
逆向思考，考虑每只猴子可能会出现在哪些位置，即对哪些位置产生贡献。
对于某只猴子 x，其最早出现的位置为 d_x，d_x 表示 x 的深度（假设根结点深度为 1）。这一点比较显然，我们从根结点一路直接走到 x 点显然最快。
再考虑它可能出现的最晚位置，一个显然的贪心想法是先走遍除以 x 为根的树之外所有的点，即其最晚出现的位置为 n- s_x+1，其中 s_x 表示以 x 为根的树的节点数量。
结论：对于猴子 x ，区间 [dx,n-s_x+1] 它一定都能走到，即它会对这部分区间产生贡献，区间加我们可以使用差分操作。
时间复杂度：O(n)。
 */