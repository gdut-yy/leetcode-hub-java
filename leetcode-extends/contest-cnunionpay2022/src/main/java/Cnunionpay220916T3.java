public class Cnunionpay220916T3 {
    public int storedEnergy(int storeLimit, int[] power, int[][] supply) {
        ...
        for (int i = 0; i < n; i++) {
            // 更新当前时刻生效的供电指令
            while (idx < supply.length && supply[idx][0] == i) {
                minSupply = supply[idx][1];
                maxSupply = supply[idx][2];
                idx++;
            }
            int p = power[i];
            if (p < minSupply) {
                // 发电量不足，用储能补到 minSupply，最多补当前储能
                int need = minSupply - p;
                if (store >= need) {
                    store -= need;
                } else {
                    store = 0;
                ...
                // 发电量超过 maxSupply，多余部分存入储能，但不能超过上限
                store += p - maxSupply;
                if (store > storeLimit) {
                    store = storeLimit;
                }
            }
            // 在 [minSupply, maxSupply] 范围内时，储能不变
        }
        return store;
    }
}
