...
public class Cnunionpay220916T4 {
    static class VendingMachine {
        private static class Batch {
            ...
            Batch(int price, int expiry, int count) {
                this.price = price;
                this.expiry = expiry;
                this.count = count;
            }
        ...
        private Map<String, List<Batch>> items;
        private Map<String, Integer> customerSuccessCount;

        public VendingMachine() {
            items = new HashMap<>();
            customerSuccessCount = new HashMap<>();
        }

        public void addItem(int time, int number, String item, int price, int duration) {
            int expiry = time + duration;
            items.computeIfAbsent(item, k -> new ArrayList<>())
                    .add(new Batch(price, expiry, number));
        }

        public long sell(int time, String customer, String item, int number) {
            List<Batch> batches = items.get(item);
            if (batches == null) {
                return -1;
            }

            // 筛选出未过期且数量大于 0 的批次
            List<Batch> available = new ArrayList<>();
            int totalAvailable = 0;
            for (Batch b : batches) {
                if (b.count > 0 && b.expiry >= time) {
                    available.add(b);
                    totalAvailable += b.count;
                }
            }
            ...
            for (Batch b : available) {
                ...
                int take = Math.min(b.count, remaining);
                originalTotal += (long) take * b.price;
                b.count -= take;
                remaining -= take;
            }

            int successCount = customerSuccessCount.getOrDefault(customer, 0);
            int discountPercent = Math.min(successCount, 30); // 最低 70% 即最多折扣 30%
            long pay = (originalTotal * (100 - discountPercent) + 99) / 100; // 向上取整

            customerSuccessCount.put(customer, successCount + 1);
            return pay;
        }
    }
 ...
 * long param_2 = obj.sell(time,customer,item,number);
 */
}
/*
银联-4. 设计自动售货机
